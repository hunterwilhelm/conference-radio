// ignore_for_file: library_private_types_in_public_api, avoid_bool_literals_in_conditional_expressions, unawaited_futures
...
class Marquee extends StatefulWidget {
  Marquee({
    ...
    this.onDone,
  })  : assert(!blankSpace.isNaN, 'The blankSpace cannot be NaN.'),
        assert(blankSpace >= 0, 'The blankSpace needs to be positive or zero.'),
        assert(blankSpace.isFinite, 'The blankSpace cannot be infinite.'),
        assert(!velocity.isNaN, 'The velocity cannot be NaN.'),
        assert(velocity != 0.0, 'The velocity cannot be zero.'),
        assert(velocity.isFinite, 'The velocity cannot be infinite.'),
        assert(
        ...
        assert(
            numberOfRounds == null || numberOfRounds > 0,
            'The numberOfRounds '
...
class _MarqueeState extends State<Marquee> with SingleTickerProviderStateMixin {
  ...
  double _getTextWidth(BuildContext context) {
    ...
    final renderObject = richTextWidget.createRenderObject(context)..layout(constraints);
